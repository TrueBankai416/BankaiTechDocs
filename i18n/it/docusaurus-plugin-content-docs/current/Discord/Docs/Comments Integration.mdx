...
* **Comunicazione bidirezionale**: Commenti sul sito web → Discord, Risposte Discord → Sito web
...
* **Database**: il database SQLite verrà creato automaticamente
...
* **Timestamp**: Quando è stato pubblicato il commento
...
* **Conversazioni a thread**: struttura commento organizzato → risposta

## Configurazione del contesto

### Livelli di contesto

Configura la quantità di contesto visualizzata nei messaggi Discord:
...
* **Caso d'uso**: Controllo preciso su ciò che appare

### Controlli contestuali individuali

Utilizzando `DISCORD_CONTEXT_LEVEL=custom`, è possibile controllare ciascun elemento:
...
DISCORD_SHOW_TIMESTAMP=true # Mostra quando è stato pubblicato il commento
...
DISCORD_SITE_NAME=Nome del tuo sito # Nome del sito nel piè di pagina
```
...
"Questo tutorial è stato davvero utile! Come posso gestire i conflitti di porta?"
```
...
```
💬 Guida all'installazione di Docker
"Questo tutorial è stato davvero utile! Come gestisco i conflitti di porta?"
[Link alla pagina]
```
...
```
💬 Nuovo commento: Guida all'installazione di Docker
👤 John Doe
"Questo tutorial è stato davvero utile! Come gestisco i conflitti di porta?"

📄 Pagina: Guida all'installazione di Docker
📍 Navigazione: Docker > Documentazione > Installazione di Docker
📂 Sezione: Docker
🔗 Percorso: /Docker/Docs/Installing%20Docker/Docker%20Engine

Nome del tuo sito • Sistema di commenti
```
...
Il componente commenti di Discord utilizza moduli CSS. È possibile personalizzare l'aspetto modificando:
...
1. **Server in esecuzione**: controlla se il server backend è in esecuzione sulla porta corretta
2. **URL API**: verifica che `REACT_APP_API_URL` punti al tuo server backend
3. **Database**: controlla se il database SQLite è in fase di creazione nella cartella del server
...
* **Sicurezza API**: valuta l'implementazione di una limitazione della velocità sugli endpoint API
* **CORS**: configura CORS correttamente per il tuo dominio di produzione

## Architettura

```
Frontend del sito web ←→ API Express ←→ Bot Discord ←→ Server Discord
                    ↓
                Database SQLite
```
...
│ ├── discord-bot.js # Logica del bot Discord
│ ├── database.js # Funzioni del database SQLite
│ └── package.json # Dipendenze del server
└── docs/Discord/Docs/ # Documentazione
```
