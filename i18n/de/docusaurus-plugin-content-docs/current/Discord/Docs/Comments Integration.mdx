---
title: Kommentarintegration
description: Vollständige Anleitung zur Integration bidirektionaler Discord-Kommentare mit Docusaurus, einschließlich Einrichtung, Konfiguration und Fehlerbehebung.
image: /img/social/discord-social.jpg
sidebar_position: 2
---

# Discord-Kommentarintegration

Diese Dokumentationsseite beinhaltet eine **bidirektionale Discord-Kommentarintegration**, die es Besuchern ermöglicht, Kommentare an Ihren Discord-Server zu senden UND Discord-Antworten auf der Website zusammen mit den bestehenden GitHub-basierten Giscus-Kommentaren anzuzeigen.

## Merkmale

- **Bidirektionale Kommunikation**: Website-Kommentare → Discord, Discord-Antworten → Website
- **Doppeltes Kommentarsystem**: Sowohl Giscus (GitHub-basiert) als auch Discord-Kommentare funktionieren zusammen
- **Erweiterte Discord-Einbettungen**: Kommentare werden als formatierte Einbettungen mit vollständigem Seitenkontext angezeigt.
- **Thread-Management**: Automatische Thread-Erstellung für übersichtliche Antworten
- **Echtzeit-Updates**: Die Website zeigt Discord-Antworten automatisch an.
- **Vollständiger Seitenkontext**: Kommentare enthalten Seitentitel, URL und Navigationskontext
- **Benutzeravatare**: Discord-Antworten zeigen Benutzeravatare und Benutzernamen an.
- **Responsives Design**: Funktioniert sowohl auf Desktop- als auch auf Mobilgeräten

## Einrichtungsanleitung

### 1. Erstelle einen Discord-Bot

1. Gehe zum [Discord Developer Portal](https://discord.com/developers/applications)
2. Neue Anwendung erstellen
3. Gehen Sie zum Bereich „Bots“ und erstellen Sie einen Bot.
4. Kopiere das Bot-Token
5. Aktivieren Sie unter „Privilegierte Gateway-Intents“ Folgendes:
   - **Inhalt der Nachricht**
   - **Absicht der Servermitglieder** (optional)

### 7. Bereitstellungskonfiguration

Ihr Bot benötigt folgende Berechtigungen:

- Nachrichten lesen
- Nachrichten senden
- Öffentliche Threads erstellen
- Nachrichten in Threads senden
- Nachrichtenverlauf lesen
- Slash-Befehle verwenden (optional)

### 6. Start the Backend Server

1. Gehen Sie zu OAuth2 > URL-Generator
2. Wählen Sie den Bereich "Bot" aus.
3. Wählen Sie die oben aufgeführten Berechtigungen aus.
4. Verwenden Sie die generierte URL, um den Bot auf Ihren Server einzuladen.

### 5. Serverabhängigkeiten installieren

#### Frontend (.env)

```env
# Discord-Bot-Konfiguration
DISCORD_BOT_TOKEN=Ihr_Bot-Token_hier
DISCORD_CHANNEL_ID=Ihre_Kanal-ID_hier

# API-Konfiguration
REACT_APP_API_URL=http://localhost:3001

# Discord-Kontextkonfiguration
DISCORD_CONTEXT_LEVEL=full
DISCORD_SHOW_BREADCRUMBS=true
DISCORD_SHOW_SECTION_INFO=true
DISCORD_SITE_NAME=Ihr Seitenname
```

#### Backend-Server (server/.env)

```env
# Discord-Bot-Konfiguration
DISCORD_BOT_TOKEN=Ihr_Bot-Token_hier
DISCORD_CHANNEL_ID=Ihre_Kanal-ID_hier

# API-Konfiguration
PORT=3001

# Discord-Kontextkonfiguration
DISCORD_CONTEXT_LEVEL=full
DISCORD_SHOW_PAGE_CONTEXT=true
DISCORD_SHOW_BREADCRUMBS=true
DISCORD_SHOW_SECTION_INFO=true
DISCORD_SHOW_PAGE_PATH=false
DISCORD_SHOW_TIMESTAMP=true
DISCORD_SHOW_AUTHOR_FIELD=true
DISCORD_CREATE_THREADS=true
DISCORD_THREAD_DURATION=1440
DISCORD_SITE_NAME=Ihr Website-Name
```

:::Warnung Sicherheitshinweis

- `.env`-Dateien niemals in die Versionskontrolle einchecken.
- Umgebungsvariablen in der Produktion verwenden
- Schützen Sie Ihr Bot-Token
  :::

### 6. Starten Sie den Backend-Server.

```bash
cd server
npm install
```

### 6. Start the Backend Server

```bash
# Entwicklung
npm run dev

# Produktion
npm start
```

### 7. 7. Deploy Configuration

Für Produktionsumgebungen müssen Sie Umgebungsvariablen in Ihrer Hosting-Plattform festlegen:

- **Frontend**: Setzen Sie `REACT_APP_API_URL` auf Ihre Produktions-API-URL.
- **Backend**: Serverordner mit Bot-Token und Kanal-ID bereitstellen
- **Datenbank**: Es wird automatisch eine SQLite-Datenbank erstellt.
- **Kontext**: Konfigurieren Sie die Discord-Kontextebene und -Optionen nach Ihren Wünschen.

## So funktioniert es

### Vollständiger bidirektionaler Fluss

1. **Website-Kommentar**: Der Nutzer sendet einen Kommentar über das Website-Formular ab.
2. **Discord-Nachricht**: Der Bot sendet formatierten Inhalt mit vollständigem Seitenkontext an den Discord-Kanal.
3. **Themenerstellung**: Der Bot erstellt automatisch einen Thread für übersichtliche Antworten.
4. **Discord-Antworten**: Discord-Nutzer antworten im Thread oder auf die Nachricht.
5. **Datenbankspeicherung**: Der Bot speichert alle Kommentare und Antworten in einer SQLite-Datenbank.
6. **Website-Anzeige**: Die Frontend-API fragt alle 30 Sekunden ab und zeigt die Discord-Antworten an.

### Discord-Nachrichtenformat

Discord-Einbettungen umfassen:

- **Title**: "💬 New Comment: \{Page Title}"
- **Beschreibung**: Der eigentliche Kommentarinhalt
- **Autor**: Name des Kommentators
- **URL**: Klickbarer Link zur entsprechenden Seite
- **Seitenfeld**: Zeigt den Seitentitel mit anklickbarem Link an
- **Zeitstempel**: Wann der Kommentar veröffentlicht wurde
- **Thema**: Automatisch erstelltes Thema für Antworten

### Website-Anzeige

Auf der Webseite wird Folgendes angezeigt:

- **Originalkommentare**: Formularübermittlungen auf der Website
- **Discord-Antworten**: Antworten von Discord-Nutzern mit Avataren und Benutzernamen
- **Echtzeit-Aktualisierungen**: Automatische Abfrage nach neuen Antworten
- **Verschachtelte Konversationen**: Strukturierte Kommentar-Antwort-Abfolge

## Kontextkonfiguration

### Kontextebenen

Konfigurieren Sie, wie viel Kontext in Discord-Nachrichten angezeigt wird:

#### `DISCORD_CONTEXT_LEVEL=minimal`

- **Angezeigt**: Nur Seitentitel und Kommentarinhalt
- **Anwendungsfall**: Klare, einfache Nachrichten ohne zusätzlichen Kontext

#### `DISCORD_CONTEXT_LEVEL=basic`

- **Angezeigt werden:** Seitentitel, Kommentarinhalt und anklickbare Seiten-URL
- **Anwendungsfall**: Moderater Kontext mit wesentlichen Informationen

#### `DISCORD_CONTEXT_LEVEL=full` (Standard)

- **Zeigt an:** Alle Kontextinformationen einschließlich Breadcrumbs, Abschnitte und Zeitstempel
- **Anwendungsfall**: Maximaler Kontext für detaillierte Diskussionen

#### `DISCORD_CONTEXT_LEVEL=custom`

- **Zeigt an**: Nur die Kontextelemente, die Sie einzeln aktivieren.
- **Anwendungsfall**: Feinabstimmung der Darstellung

### Individuelle Kontextsteuerung

Bei Verwendung von `DISCORD_CONTEXT_LEVEL=custom` können Sie jedes Element steuern:

```env
DISCORD_SHOW_PAGE_CONTEXT=true # Seitentitel und URL-Feld anzeigen
DISCORD_SHOW_BREADCRUMBS=true # Navigations-Breadcrumbs anzeigen
DISCORD_SHOW_SECTION_INFO=true # Hauptbereich anzeigen (Docker, Nextcloud usw.)
DISCORD_SHOW_PAGE_PATH=false # Vollständigen URL-Pfad anzeigen
DISCORD_SHOW_TIMESTAMP=true # Datum der Kommentarabgabe anzeigen
DISCORD_SHOW_AUTHOR_FIELD=true # Namen des Kommentators als Einbettungsautor anzeigen
```

### Thread-Konfiguration

```env
DISCORD_CREATE_THREADS=true # Threads für Antworten erstellen
DISCORD_THREAD_DURATION=1440 # Automatische Archivierungszeit der Threads (Minuten)
DISCORD_SITE_NAME=Ihr Seitenname # Seitenname in der Fußzeile
```

### Beispielhafte Kontextausgaben

**Minimaler Kontext:**

```
Docker-Installationsanleitung
"Dieses Tutorial war wirklich hilfreich! Wie gehe ich mit Portkonflikten um?"
```

**Grundlegender Kontext:**

```
💬 Docker-Installationsanleitung
"Dieses Tutorial war wirklich hilfreich! Wie gehe ich mit Portkonflikten um?"
[Link zur Seite]
```

**Vollständiger Kontext:**

```
💬 Neuer Kommentar: Docker-Installationsanleitung
👤 John Doe
"Dieses Tutorial war wirklich hilfreich! Wie gehe ich mit Portkonflikten um?"

📄 Seite: Docker-Installationsanleitung
📍 Navigation: Docker > Dokumentation > Docker installieren
📂 Abschnitt: Docker
🔗 Pfad: /Docker/Docs/Installing%20Ddocker/Docker%20Engine

Ihr Website-Name • Kommentarsystem
```

## Anpassung

### Styling

Die Discord-Kommentarkomponente verwendet CSS-Module. Sie können das Erscheinungsbild anpassen, indem Sie Folgendes ändern:

```
src/components/DiscordComments/styles.module.css
```

### Nachrichtenformat

Um das Discord-Einbettungsformat anzupassen, bearbeiten Sie das `discordMessage`-Objekt in:

```
src/components/DiscordComments/index.tsx
```

### Bauteilplatzierung

Die Discord-Kommentare werden standardmäßig unterhalb der Giscus-Kommentare angezeigt. Um dies zu ändern, gehen Sie wie folgt vor:

```
src/theme/DocItem/Layout/index.js
```

## Fehlerbehebung

### Bot antwortet nicht

1. **Bot-Token prüfen**: Stellen Sie sicher, dass `DISCORD_BOT_TOKEN` korrekt ist.
2. **Berechtigungen prüfen**: Stellen Sie sicher, dass der Bot über die erforderlichen Berechtigungen im Kanal verfügt.
3. **Kanal-ID prüfen**: Stellen Sie sicher, dass `DISCORD_CHANNEL_ID` korrekt ist.
4. **Bot auf dem Server**: Überprüfen Sie, ob sich der Bot tatsächlich auf Ihrem Discord-Server befindet.

### Kommentare werden nicht angezeigt

1. **Server läuft**: Überprüfen Sie, ob der Backend-Server auf dem richtigen Port läuft.
2. **API-URL**: Überprüfen Sie, ob `REACT_APP_API_URL` auf Ihren Backend-Server verweist.
3. **Datenbank**: Prüfen Sie, ob im Serverordner eine SQLite-Datenbank erstellt wird.
4. **CORS**: Stellen Sie sicher, dass CORS für Ihre Frontend-Domain konfiguriert ist.

### Discord-Antworten werden nicht angezeigt

1. **Nachrichteninhaltsabsicht**: Stellen Sie sicher, dass diese Option im Discord-Entwicklerportal aktiviert ist.
2. **Bot-Berechtigungen**: Sicherstellen, dass der Bot Nachrichten im Kanal lesen kann
3. **Datenbankverbindung**: Überprüfen Sie die Serverprotokolle auf Datenbankfehler.
4. **Umfragen**: Die Benutzeroberfläche führt alle 30 Sekunden Umfragen durch – es kann eine Weile dauern, bis die Antworten erscheinen.

### Umgebungsvariablen funktionieren nicht

- **Entwicklung**: Starten Sie sowohl die Frontend- als auch die Backend-Server neu.
- **Produktion**: Frontend und Backend mit den korrekten Umgebungsvariablen erneut bereitstellen.
- **Präfix**: Verwenden Sie das Präfix `REACT_APP_` nur für Frontend-Variablen.

### Datenbankprobleme

1. **Dateiberechtigungen**: Stellen Sie sicher, dass der Server in das Datenbankverzeichnis schreiben kann.
2. **Speicherplatz**: Prüfen Sie den verfügbaren Speicherplatz für die SQLite-Datenbank.
3. **Serverprotokolle**: Überprüfen Sie die Serverkonsole auf Datenbankverbindungsfehler.

## Sicherheitsüberlegungen

- **Bot-Token**: Halten Sie Ihren Bot-Token geheim und speichern Sie ihn niemals in der Versionskontrolle.
- **Ratenbegrenzung**: Discord hat Ratenbegrenzungen für Bots (anders als für Webhooks).
- **Eingabevalidierung**: Sowohl Frontend als auch Backend validieren die Benutzereingaben.
- **Datenbanksicherheit**: Die SQLite-Datenbank sollte durch entsprechende Dateiberechtigungen geschützt werden.
- **API-Sicherheit**: Erwägen Sie die Implementierung einer Ratenbegrenzung für Ihre API-Endpunkte.
- **CORS**: Konfigurieren Sie CORS ordnungsgemäß für Ihre Produktionsdomäne.

## Architektur

```
Website-Frontend ←→ Express-API ←→ Discord-Bot ←→ Discord-Server
                    ↓
                SQLite-Datenbank
```

## Dateistruktur

```
project/
├── src/components/DiscordComments/ # Frontend-Komponente
├── server/ # Backend-Server
│ ├── api.js # Express-API-Server
│ ├── discord-bot.js # Discord-Bot-Logik
│ ├── database.js # SQLite-Datenbankfunktionen
│ └── package.json # Serverabhängigkeiten
└── docs/Discord/Docs/ # Dokumentation
```

## Browserunterstützung

Die Discord-Kommentarfunktion funktioniert in allen modernen Browsern, die Folgendes unterstützen:

- ES6 Fetch-API
- React 18+
- CSS Grid und Flexbox
- Lokaler Speicher (zur Formularpersistenz)
