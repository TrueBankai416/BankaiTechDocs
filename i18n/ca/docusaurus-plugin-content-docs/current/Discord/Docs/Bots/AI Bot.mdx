...
description: Step-by-step guide for creating a Discord AI bot using Python, OpenAI GPT-3.5 Turbo, and Discord.py with systemd service configuration.
...
7. Navigate to the "Installation" tab on the left side, click on the Install Link dropdown and select `None` and save the changes.
