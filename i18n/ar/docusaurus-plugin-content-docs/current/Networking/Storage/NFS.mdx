...
# Install NFS server packagessudo apt install nfs-kernel-server nfs-common# Start and enable NFS servicessudo systemctl start nfs-kernel-server
sudo systemctl enable nfs-kernel-server# Check service statussudo systemctl status nfs-kernel-server```
...
# Install NFS utilities (package name varies by distribution)# Common package names: nfs-kernel-server, nfs-utils, nfs-server# Start NFS servicessudo systemctl start nfs-server
...
# Start and enable NFS servicessudo systemctl start nfs-server
sudo systemctl enable nfs-server
sudo systemctl start rpcbind
sudo systemctl enable rpcbind# Check service statussudo systemctl status nfs-server```
