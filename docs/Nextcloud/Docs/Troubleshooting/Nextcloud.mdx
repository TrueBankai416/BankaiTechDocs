---
title: Nextcloud
description: Common Nextcloud troubleshooting solutions including untrusted domain errors, missing database indexes, WebSocket notify_push errors, and configuration fixes.
image: /img/social/nextcloud-social.webp
sidebar_position: 1
tags: [Docker, Nextcloud, Tutorial, Troubleshooting]
keywords: [Docker, Docker Engine, Tutorial, Nextcloud, Troubleshooting]
last_update:
  author: BankaiTech
updates:
  - date: 2026-08-07
    note: "Add troubleshooting section for WebSocket connection error caused by misconfigured notify_push app"
  - date: 2026-01-02
    note: "Add troubleshooting section for upstream timeout errors during large file uploads/downloads in Nextcloud"
---

import BuyMeACoffeeButton from '@site/src/components/BuyMeACoffeeButton';

## **Troubleshooting for Nextcloud**

### **_Access through untrusted Domain_**

<p>
<>This happens when you attempt to access nextcloud from a source not listed inside the `config.php` file.</>
</p>
![NC Untrusted Domain](/img/nextcloud/Troubleshooting/untrustedDomain.jpg)

#### _Changing directories_

<p>
<>First we need to change our directory to where nextcloud is installed</>
</p>
In this example, nextclouds webroot is located at `/var/docker/nextcloud/nextcloud/`
```bash
cd /var/docker/nextcloud/nextcloud
```
:::info
`cd` stands for `Change Directory`
:::

<p>
<>If you are in the right spot then you will see</>
</p>
```bash
3rdparty  apps           config       cron.php     dist        lib                       ocs                package.json  resources   themes
AUTHORS   composer.json  console.php  custom_apps  index.html  nextcloud-init-sync.lock  ocs-provider       public.php    robots.txt  version.php
COPYING   composer.lock  core         data         index.php   occ                       package-lock.json  remote.php    status.php
```

#### _Editing the config.php file_

<p>
<>Now lets edit the `config.php` file using nano</>
</p>
```bash
nano config/config.php
```
<p>
<>Scroll down until you see</>
</p>
```editorconfig
'trusted_domains' =>
array (
    0 => '',
  ),
```

#### _Adding your domain to config.php_

<p>
<>Now add your domain to the array</>
</p>
```editorconfig
'trusted_domains' => 
  array (
    0 => 'cloud.bankai-tech.com',
  ),
```
<p>
<>If you need to add multiple domains you can do so like this</>
</p>
```editorconfig
'trusted_domains' => 
  array (
    0 => 'cloud.bankai-tech.com',
    1 => '192.168.4.130',
  ),
```
<p>
<>Now save the file and you should be able to access nextcloud now</>
</p>
:::tip
Save the file by pressing `CTRL+X`
:::note
If the error doesnt go away, you may need to clear your browser cache and restart the browser
:::

### **_The database is missing some indexes._**

![Missing Indexes](/img/nextcloud/Troubleshooting/missingIndexes.jpg)

<p>
<>This seems to be caused by nextcloud skipping large indexes. To fix in a Docker environment, run the following.</>
</p>
```bash
sudo docker exec -it -u 33 nextcloud php occ db:add-missing-indices
```

### **_Upstream Timed Out Error on Large File Uploads/Downloads_**

<p>
<>If you encounter this error when uploading or downloading large files:</>
</p>
```
[error] 412#412: *464 upstream timed out (110: Operation timed out) while reading response header from upstream
```
<p>
<>This error typically occurs when Nginx times out while waiting for a response from the PHP-FPM upstream when handling large files. The solution is to disable the content length header in Nextcloud's configuration.</>
</p>

#### _Editing the config.php file_

<p>
<>Navigate to your Nextcloud installation directory and edit the `config.php` file:</>
</p>
```bash
cd /opt/docker/nextcloud/nextcloud
nano config/config.php
```
<p>
<>Add the following line to your config array:</>
</p>
```php
'set_time_limit' => 3600,
'set_header_content_length' => false,
```
:::info
The `'set_header_content_length' => false,` setting prevents Nextcloud from setting the Content-Length header, which can cause issues with large file transfers through reverse proxies.
:::
<p>
<>After adding this configuration, save the file and restart your Nextcloud container:</>
</p>
```bash
sudo docker restart nextcloud
```
:::tip
If you continue to experience issues, you may also need to increase timeout values in your Nginx configuration:
- `proxy_read_timeout`
- `proxy_connect_timeout`
- `proxy_send_timeout`
:::

### **_WebSocket Connection Error (notify_push)_**

<p>
<>If you see the following error in the browser console:</>
</p>
```
Firefox can't establish a connection to the server at ws://localhost:7867/ws.
```
<p>
<>This is caused by the `notify_push` setup command never being run, so clients default to connecting directly to `localhost:7867`. A secondary cause is having `NEXTCLOUD_URL=http://localhost:80` in your `.env` file — this should be your actual domain URL. Run the following command to register the correct push endpoint:</>
</p>
```bash
sudo docker exec -u www-data nextcloud php occ notify_push:setup https://your-domain.com/push
```
:::info
Replace `your-domain.com` with your actual Nextcloud domain (e.g. `cloud.bankai-tech.com`).
:::

## **Example Files**

### **_Config.php_**

```editorconfig
<?php
$CONFIG = array (
  'htaccess.RewriteBase' => '/',
  'memcache.local' => '\\OC\\Memcache\\APCu',
  'apps_paths' =>
  array (
    0 =>
    array (
      'path' => '/var/www/html/apps',
      'url' => '/apps',
      'writable' => false,
    ),
    1 =>
    array (
      'path' => '/var/www/html/custom_apps',
      'url' => '/custom_apps',
      'writable' => true,
    ),
  ),
  'memcache.distributed' => '\\OC\\Memcache\\Redis',
  'memcache.locking' => '\\OC\\Memcache\\Redis',
  'redis' =>
  array (
    'host' => 'redis',
    'password' => '',
    'port' => 6379,
  ),
  'trusted_proxies' =>
  array (
    0 => '192.168.4.204',
  ),
  'forwarded-for-headers' =>
  array (
    0 => 'HTTP_X_FORWARDED_FOR',
  ),
  'upgrade.disable-web' => true,
  'instanceid' => '**REDACTED**',
  'passwordsalt' => '**REDACTED**',
  'secret' => '**REDACTED**',
  'trusted_domains' =>
  array (
    0 => 'cloud.bankai-tech.com',
    1 => '192.168.4.130',
  ),
  'datadirectory' => '/var/www/html/data',
  'dbtype' => 'mysql',
  'version' => '30.0.2.2',
  'dbname' => 'nextcloud',
  'dbhost' => 'mariadb',
  'dbport' => '',
  'dbtableprefix' => 'oc_',
  'mysql.utf8mb4' => true,
  'dbuser' => 'nextcloud',
  'dbpassword' => 'CHANGEME',
  'installed' => true,
  'maintenance_window_start' => 1,
  'default_phone_region' => 'US',
  'overwriteprotocol' => 'https',
  'overwrite.cli.url' => 'https://cloud.bankai-tech.com',
  'loglevel' => 2,
);
```

## **Clearing the Logs**

### _Nextcloud_

```bash
sudo docker exec -it <container_name> truncate data/nextcloud.log -s 0 && sudo docker restart <container_name>
```

### _Nextcloud AIO_

```bash
sudo docker exec -it nextcloud-aio-nextcloud truncate data/nextcloud.log -s 0 && sudo docker restart nextcloud-aio-nextcloud
```

## **Resources**

<BuyMeACoffeeButton />
