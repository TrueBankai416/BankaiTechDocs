...
| Federated call is one-way — remote can see/hear you but you cannot see/hear them | Their TURN server is advertising a Docker bridge IP (e.g. `172.19.0.x`) as the relay address. This happens when coturn runs on a host that also has Docker installed — coturn binds to all interfaces by default, including Docker bridge interfaces, and advertises them as relay candidates. Your coturn logs will show `A peer IP x.x.x.x denied in the range: 172.16.0.0-172.31.255.255`. Fix: add `listening-ip=<their-LAN-IP>` to their `turnserver.conf` to restrict coturn to the real interface, and set `external-ip=<WAN>/<LAN>` so the correct address is advertised |
