...
| Federated call is one-way — remote can see/hear you but you cannot see/hear them | Their TURN server is advertising a Docker bridge IP (e.g. `172.19.0.x`) as the relay address. Your coturn logs will show `A peer IP x.x.x.x denied in the range: 172.16.0.0-172.31.255.255`. Their coturn needs `external-ip=<WAN>/<LAN>` in `turnserver.conf` and `command: ["--log-file=stdout"]` to stop `detect-external-ip` from overriding it — the exact same fix documented above for your own coturn |
