...
Talk federation is currently in **beta**. Expect rough edges and check the [Nextcloud Talk release notes](https://nextcloud.com/changelog/#latest29) for updates on what is supported in your version.
:::

Nextcloud Talk supports **federated text chat** between users on different Nextcloud instances using the [Open Cloud Mesh (OCM)](https://nextcloud.com/blog/how-federated-chat-works-in-nextcloud-talk/) standard. Federated users are added via their Federated Cloud ID (`user@cloud.example.com`) and receive an invite notification before they can join.
