...
updates:
  - date: 2026-08-06
    note: "docs: add Docker bridge interfaces to Janus ice_ignore_list"
  - date: 2026-08-06
    note: "docs: fix one-way federated call remedy to use relay-ip instead of listening-ip"
  - date: 2026-08-06
    note: "docs: correct one-way federated call cause to bare-metal coturn binding to Docker bridge interfaces"
  - date: 2026-08-06
    note: "docs: update one-way federated call entry with confirmed Docker bridge IP cause"
  - date: 2026-08-06
    note: "docs: add one-way federated media and no_such_room troubleshooting entries"
---
...
| Federated call is one-way — remote can see/hear you but you cannot see/hear them | Their TURN server is advertising a Docker bridge IP (e.g. `172.19.0.x`) as the relay address. This happens when coturn runs on a host that also has Docker installed and `relay-ip` is not set — coturn creates relay sockets on all interfaces including Docker bridges. Your coturn logs will show `A peer IP x.x.x.x denied in the range: 172.16.0.0-172.31.255.255`. Fix: ensure their `turnserver.conf` has both `external-ip=<WAN>/<LAN>` and `relay-ip=<LAN>` so relay sockets are bound only to the real interface |
