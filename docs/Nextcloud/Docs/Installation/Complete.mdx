---
title: Complete Install
description: Complete Nextcloud installation with Docker including Redis, MariaDB, Nextcloud Memories, Collabora Office, and all essential components.
image: /img/social/nextcloud-social.webp
sidebar_position: 4
tags: [Docker, Docker Compose, Tutorial, Nextcloud, Collabora Code, Nextcloud Office]
keywords: [Docker, Docker Engine, Tutorial, Nextcloud]
last_update:
  author: BankaiTech
updates:
  - date: 2026-08-07
    note: "docs(nextcloud): add HaRP AppAPI reverse proxy installation and configuration guide"
  - date: 2026-08-07
    note: "docs(nextcloud): fix notify_push WebSocket error - add setup command and correct NEXTCLOUD_URL env"
  - date: 2026-08-05
    note: "feat(docs): add Nextcloud Talk HPB documentation"
  - date: 2026-08-05
    note: "docs(nextcloud): add caution about non-functional facial recognition and setup instructions for Notify Push"
  - date: 2026-07-26
    note: "fix(docs): update MariaDB version to 11.8 in installation guides"
---

import BuyMeACoffeeButton from '@site/src/components/BuyMeACoffeeButton';

:::tip[Nextcloud Talk HPB]
Want high-quality video calls with more than 4–5 participants? Add the [Talk High Performance Backend](../Talk.mdx) after completing this install.
:::

:::tip[HaRP — AppAPI Reverse Proxy]
Want to use AI ExApps (Assistant, Memories indexing, etc.)? Set up [HaRP](../HaRP.mdx) after completing this install.
:::

:::caution[Facial Recognition]
Facial recognition is currently non-functional until further notice.
:::

## **Nextcloud Installation using Docker Build and Docker Compose**

**_This method includes Redis, MariaDB, Nextcloud Memories, and Office_**
:::note
inotify libraries have been added to the Dockerfile for the iNotify App
:::
:::info
Nextcloud 28.0.4 was the latest version when making this tutorial.
:::

First lets update your system.

```bash
sudo apt-get update && apt-get upgrade -y
```

### **_Setting up the directories_**

Now lets setup the Directories
:::note

You can change the directories to your liking.

:::

```bash
sudo mkdir /opt/docker && sudo mkdir /opt/docker/nextcloud
```

Lets take ownership of the directory

```bash
sudo chown -R $USER:$USER /opt/docker
```

Enter the Directory

```bash
cd /opt/docker/nextcloud
```

### **_Creating .env files_**

Now lets create the [Docker Variables](https://docs.docker.com/compose/how-tos/environment-variables/set-environment-variables/#use-the-env_file-attribute) files in the new directory.

### **_Creating .env_**

```bash
nano .env
```

:::warning[warning]
When editing the configuration below, remove all instances of `<>` from the values
:::
Paste the Following with `CNTRL+SHIFT+V`

```editorconfig title="Highlighted items will need to be modified" showLineNumbers
# Nextcloud and MariaDB
# Named Docker volumes for persistent data storage
DATA_LOCATION=nextcloud_data
DB_DATA_LOCATION=db_data
# Nextcloud Port
HTTP_PORT=8080
// highlight-start
NEXTCLOUD_TRUSTED_DOMAINS=cloud.<YourDomain>.com
TRUSTED_PROXIES=<IP address of your Reverse Proxy>
# Escape dots in the reverse proxy IP address, for example: ^192\.168\.1\.10$
OVERWRITECONDADDR=^<IP address of your Reverse Proxy>$
// highlight-end
OVERWRITEPROTOCOL=https
// highlight-next-line
OVERWRITECLIURL=https://cloud.<YourDomain>.com
REDIS_HOST=redis
REDIS_PORT=6379
PHP_MEMORY_LIMIT=512M
PHP_UPLOAD_LIMIT=100M
// highlight-start
NEXTCLOUD_ADMIN_USER=admin
NEXTCLOUD_ADMIN_PASSWORD=password
MYSQL_ROOT_PASSWORD=CHANGEME
MYSQL_PASSWORD=CHANGEME
// highlight-end
MYSQL_DATABASE=nextcloud
MYSQL_USER=nextcloud
MARIADB_AUTO_UPGRADE=true
# Go_Vod
// highlight-next-line
NEXTCLOUD_HOST=https://cloud.example.com
NEXTCLOUD_ALLOW_INSECURE=1 # (self-signed certs or no HTTPS)
NVIDIA_VISIBLE_DEVICES=all
# Notify Push
// highlight-next-line
NEXTCLOUD_URL=https://cloud.example.com
# Collabora
// highlight-start
username=admin
password=PASSWORD
domain=cloud\\.domain\\.com
// highlight-end
```

:::tip[Info]
Please Change `CHANGEME` to a password of your choosing.
:::danger
Refrain from using Special characters for you MySQL Passwords. \
Ex. `‘ ~ ! @ # $ % ^ & * ( ) _ - + = { } [ ] / < > , . ; ? ' : | (space)`
:::

Save the new file by pressing `CTRL+X`

### **_Creating the Compose file_**

```bash
nano docker-compose.yaml
```

Now lets edit the compose file.

```yaml title="The highlighted lines are Optional and can be removed completely"
name: nextcloud

services:
  mariadb:
    image: mariadb:11.8
    container_name: mariadb
    restart: always
    command: --transaction-isolation=READ-COMMITTED --log-bin=binlog --binlog-format=ROW --log_bin_trust_function_creators=true
    volumes:
      - ${DB_DATA_LOCATION:-db_data}:/var/lib/mysql
    env_file:
      - .env
    networks:
      - nextcloud

  nextcloud:
    image: bankaitech/nextcloud:latest
    container_name: nextcloud
    restart: always
    ports:
      - ${HTTP_PORT:-8080}:80
    links:
      - mariadb
      - redis
      - collabora
#      - onlyoffice
      - go-vod
    volumes:
      - ${DATA_LOCATION:-nextcloud_data}:/var/www/html
    env_file:
      - .env
    networks:
      - nextcloud

  cron:
    image: bankaitech/nextcloud:latest
    container_name: cron
    restart: always
    volumes:
      - ${DATA_LOCATION:-nextcloud_data}:/var/www/html:z
    depends_on:
      - mariadb
      - redis
    networks:
      - nextcloud
    entrypoint: /cron.sh

  redis:
    container_name: redis
    image: redis:latest
    // highlight-start
    volumes:            # Optional
      - type: tmpfs     # Optional
        target: /data   # Optional
        tmpfs:
          size: 1073741824 # 1GB "Optional"
    // highlight-end
    expose:
      - 6379
    command: redis-server --save 60 1 --loglevel warning
    networks:
      - nextcloud
    restart: always

  collabora:
    container_name: collabora
    hostname: collabora
    privileged: true
    tty: true
    ports:
      - 9980:9980
    cap_add:
      - MKNOD
    image: collabora/code:latest
#    volumes:
#      - ./collabora/config/coolwsd.xml:/etc/coolwsd/coolwsd.xml
    env_file:
      - .env
    networks:
      - nextcloud
    restart: always

  go-vod:
    image: radialapps/go-vod
    restart: always
    container_name: go-vod
    env_file:
      - .env
    volumes:
      - ${DATA_LOCATION:-nextcloud_data}:/var/www/html:ro
    networks:
      - nextcloud
#    runtime: nvidia # (NVENC)

#  documentserver:
#    container_name: onlyoffice
#    stdin_open: true
#    tty: true
#    ports:
#      - 87:80
#    volumes:
#      - ./DocumentServer/logs:/var/log/onlyoffice
#      - ./DocumentServer/data:/var/www/onlyoffice/Data
#    networks:
#      - nextcloud
#    image: onlyoffice/documentserver

networks:
  nextcloud:
    driver: bridge

volumes:
  nextcloud_data:
  db_data:
```

:::info[NVIDIA HWA]
If you want to use GPU for transcoding then uncomment this line\
runtime: nvidia # (NVENC) # For Nvidia HW Acceleration, Uncomment after installing nvidia drivers.\
Test nvidia by running `sudo nvidia-smi`
:::

:::warning
tmpfs uses memory (RAM), adjust the value as needed or optionally remove the volume section from the redis container
:::

## **Starting Nextcloud**

Now lets start it up.

```bash
sudo docker compose up -d
```

:::info

the `-d` flag stands for [detatched](https://docs.docker.com/engine/reference/run/#foreground-and-background) mode.
:::

<p>
<>After its done pulling the docker images you can open up a web browser.</>
</p>
:::note
Before proceeding, you will need to configure your reverse proxy. I have an example Nginx config [Here](/Examples/Reverse%20Proxies/Docs/Nginx/#nextcloud).
 - Additional Documentation: [Reverse Proxies](/Reverse%20Proxy/Introduction) 
:::
<p>
<>Connect to nextcloud by typing in your configured `domain`</>
</p>

<p>
<>You should see this screen.</>
</p>

![Nextcloud First Start](/img/nextcloud/FirstStart.jpg)

:::warning
If you see this warning

```

Performance warning

You chose SQLite as database.

SQLite should only be used for minimal and development instances. For production we recommend a different database backend.

If you use clients for file syncing, the use of SQLite is highly discouraged.
```

You will need to click the drop down and select `MySQL/MariaDB` and type in the info manually.

- **database account:** nextcloud
- **database password:** YOUR_PASSWORD_FROM .env
- **database name:** nextcloud
- **database host:** mariadb:3306
  :::

### **_Account Creation_**

<p>
<>Now create an admin account and click `Install`</>
</p>

<p>
<>Click `Install Recommended Apps`</>
</p>

![Nextcloud Install Recommended Apps](/img/nextcloud/FirstStart_RecommendedApps.jpg)

<p>
<>When its done installing you should see your Dashboard</>
</p>

![Nextcloud Dashboard](/img/nextcloud/Dashboard.jpg)

### **_Checking for errors and warnings_**

<p>
<>On the top right click the `User Icon` and go to `Administration Settings`</>
</p>

![Nextcloud Administration Settings](/img/nextcloud/Administration_Settings.jpg)

<p>
<>Here you should see `Security & Setup Warnings`</>
</p>

![Nextcloud Warnings](/img/nextcloud/Warnings.jpg)

<p>
<>In order to fix the [Maintenance Window](https://docs.nextcloud.com/server/latest/admin_manual/configuration_server/config_sample_php_parameters.html#maintenance-window-start) and the [Phone Region](https://docs.nextcloud.com/server/latest/admin_manual/configuration_server/config_sample_php_parameters.html#default-phone-region) warnings, we will have to edit our `config.php` file.</>
</p>

### **_Modifying the config.php file_**

Run the following command to open the `config.php` file with nano.

```bash
nano nextcloud/config/config.php
```

:::tip[Hint]

<p>
<>If you recieved an error `Directory nextcloud doesnt exist`, then you need to change directories.</>
</p>
Run `cd /opt/docker/nextcloud` then try again.
:::

Now add the following to the bottom of the file.

```editorconfig title="Highlighted items can be changed to your Time Zone Identifier" {2} showLineNumbers
  'maintenance_window_start' => 1,
  'default_phone_region' => 'US',
```

![Nextcloud config.php](/img/nextcloud/config.jpg)

<p>
<>Now if you go back to your `Administration Settings` Page.</>
</p>

<p>
<>It should look something like this.</>
</p>

![Nextcloud No Warnings](/img/nextcloud/NoErrors.jpg)

:::tip[HINT]

In some cases, you may have to delete your browser cache and restart the browser for the Warnings to update.
:::

### **_Creating production use user_**

<p>
<>Now you can go create a user for yourself.</>
</p>

<p>
<>Go to the user icon on the top right and select `Users`.</>
</p>

![Nextcloud Users](/img/nextcloud/users.jpg)

<p>
<>Click on `Add User` on the left hand side.</>
</p>

![Nextcloud Add Users](/img/nextcloud/Adduser.jpg)

<p>
<>Now create your User.</>
</p>
:::warning[Caution]

Make sure to set `Groups` to `admin`
:::

![Nextcloud Add Users1](/img/nextcloud/Adduser1.jpg)

## **Setting up Notify Push**

`notify_push` is included in the `bankaitech/nextcloud` image and provides instant client push notifications for real-time file sync.

After starting the containers, run the following commands:

Add `127.0.0.1` to your trusted proxies to avoid CSP issues:
```bash
docker exec -it -u 33 nextcloud php occ config:system:set trusted_proxies 1 --value="127.0.0.1"
```

Then register your domain with the push server so clients connect to the correct endpoint:
```bash
sudo docker exec -u www-data nextcloud php occ notify_push:setup https://cloud.example.com/push
```
:::info
Replace `cloud.example.com` with your actual Nextcloud domain. Without this step, clients will attempt to connect to `ws://localhost:7867/ws` and fail.
:::

## **Setting up Nextcloud Office (Collabora Code)**

Head back over to your `Administration Settings` and scroll down on the left until you see `Nextcloud Office`
Wait a few seconds while it processes.
Now choose `Use your own Server`
![Choose your Server](/img/nextcloud/OfficeServer.jpg)

<p>
<>Enter in your domain that you will use for office and Uncheck `Disable certificate verification`</>
</p>
:::note
You will have to add an `A` record for `office` and setup your reverse proxy for Collabra
:::
![CollaboraUrl](/img/nextcloud/OfficeUrl.jpg)

### **_WOPI List_**

<p>
<>Now scroll down to the `WOPI List`</>
</p>
Enter in your `Public IP`, Network `CIDR`, and `127.0.0.1`
:::info
Ex. `251.163.184.3,192.168.1.1/24,127.0.0.1`
:::
![Office WOPI](/img/nextcloud/OfficeWOPI.jpg)
Press `ENTER`

### **_Testing Nextcloud Office_**

<p>
<>Go to `Files` and create a new document</>
</p>
![New Document](/img/nextcloud/NewDocument.jpg)
![Nextcloud Office](/img/nextcloud/NextcloudOffice.jpg)

## **Installing Nextcloud Memories**

<p>
<>Go to `Apps`</>
</p>
![Nextcloud Apps](/img/nextcloud/Apps.jpg)
<p>
<>Now click `Download and enable`</>
</p>
![Enable Memories](/img/nextcloud/InstallMemories.jpg)
<p>
<>Now search and install `Preview Generator`.</>
</p>

### **_Configuring Nextcloud Memories_**

<p>
<>Go to your `Administration Settings` and find `Memories`</>
</p>
![Memories](/img/nextcloud/Memories.jpg)
<p>
<>My Settings are as Follows</>
</p>
![Memories Setting1](/img/nextcloud/MemoriesSetting1.jpg)
![Memories Setting2](/img/nextcloud/MemoriesSetting2.jpg)
![Memories Setting3](/img/nextcloud/MemoriesSetting3.jpg)
![Memories Setting4](/img/nextcloud/MemoriesSetting4.jpg)
![Memories Setting5](/img/nextcloud/MemoriesSetting5.jpg)
![Memories Setting6](/img/nextcloud/MemoriesSetting6.jpg)

### **_Fixing the warnings_**

<p>
<>Im sure you noticed that there are a few warnings we need to take care of.</>
</p>
```
 Geometry table has not been created.
Reverse geocoding has not been configured (-1). 
```
<p>
<>We can generate the Geometry table using this command.</>
</p>
```bash
sudo docker exec -it -u 33 nextcloud php occ memories:places-setup
```
:::tip
I prefer generating the Geometry table this way so I can view the progress
:::

## **Congragulations!! you have installed Nextcloud!**

## **_Troubleshooting_**

<p>
<>For troubleshooting, please visit [Troublshooting](../Troubleshooting/Nextcloud.mdx).</>
</p>

<p>
<>Comment at the bottom of this page if you need help or have any suggestions to improve this site</>
</p>

<BuyMeACoffeeButton />
