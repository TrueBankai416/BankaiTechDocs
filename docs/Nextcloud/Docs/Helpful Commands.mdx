...
tags: [Nextcloud, occ, Commands, Administration, Docker]
keywords: [Nextcloud, occ, commands, administration, Docker, maintenance]
last_update:
  author: BankaiTech
updates:
  - date: 2026-07-26
    note: "docs(nextcloud): add command to hide simple signup link in OCC commands"
---
...
### **_Generating a redacted list of your config.php_**
...
#### _Show mastercontainer password for Nextcloud AIO_
...
### **_Generate Geometry Table for Memories_**
...
### **_Hide Simple Signup Link_**

This OCC command hides the public simple signup link prompt in Nextcloud by setting the `simpleSignUpLink.shown` system value to `false`.

```bash
sudo docker compose exec -u www-data nextcloud php occ config:system:set simpleSignUpLink.shown --type=boolean --value=false
```
...
### **_Scan files for a specific user_**

```bash
sudo docker exec -it -u 33 nextcloud php occ files:scan <user>
```

### **_Reset BruteForce Detection_**
