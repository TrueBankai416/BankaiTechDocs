...
# Check MergeFS process
if ! pgrep mergerfs > /dev/null; then
    log_message "ERROR: MergeFS process not running"
fi
...
log_message "Starting MergeFS maintenance"
...
# Test MergeFS pool performance
echo "Testing MergeFS pool:"
...
### **1. Adding MergeFS Storage to Proxmox**

```bash
# Add MergeFS pool as Proxmox storage
...
```bash
# Create VM with storage on MergeFS pool
...
# Move existing VM to MergeFS storage
...
```bash
# Configure backup to use MergeFS storage
pvesm add dir mergerfs-backup --path /mnt/storage/backup --content backup

# Create backup job using MergeFS storage
...
- **Mix drive sizes freely** - MergeFS handles different capacities well
- **Consider drive failure patterns** - avoid drives from the same batch
- **Use enterprise drives** for 24/7 operation
- **Implement external backup** - MergeFS provides no redundancy
