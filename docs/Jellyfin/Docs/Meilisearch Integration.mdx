...
updates:
  - date: 2026-07-08
    note: "docs: harden Meilisearch restore and pin image versions"
  - date: 2026-01-05
    note: "Update documentation to standardize file extension from .yml to .yaml for Docker Compose"
  - date: 2026-01-05
    note: "Add import statement for BuyMeACoffeeButton component in Meilisearch integration documentation"
  - date: 2026-01-05
    note: "Refactor code structure for improved readability and maintainability"
  - date: 2026-01-05
    note: "Update Meilisearch integration documentation to include instructions for using the latest image tag in docker-compose"
---
...
2. **Update your `docker-compose.yaml`** to use the older image:
...
:::tip[Finding the master key on TrueNAS]
If you are running this on TrueNAS and cannot find your Meilisearch master key, run:

```bash
sudo docker inspect meilisearch | grep MEILI_MASTER_KEY
```

Use the value shown for the `Authorization: Bearer ...` header below.
:::
...
If you need to access the volume directly, first confirm the exact volume mounted to `/meili_data`:

```bash
sudo docker inspect meilisearch --format '{{range .Mounts}}{{if eq .Destination "/meili_data"}}{{.Name}}{{end}}{{end}}'
```

If your Compose file uses a project name (for example `name: media-management`), Docker may prefix the volume name (for example `media-management_meilisearch_data`). Use the exact mounted volume name from the inspect command in all `docker run -v ...` and `docker volume inspect ...` commands.

Then inspect that volume using:

```bash
sudo docker volume inspect <mounted-volume-name>
```
...
Update your `docker-compose.yaml` to use a pinned version tag (do not use `latest`):
...
Replace `v1.49.0` with the exact version you validated for your data and migration path.

Then pull and start that specific version:

```bash
# Pull the pinned image version
sudo docker pull getmeili/meilisearch:v1.49.0
...
1. **Add the import environment variable** to your `docker-compose.yaml`:
...
VOL=$(sudo docker inspect meilisearch --format '{{range .Mounts}}{{if eq .Destination "/meili_data"}}{{.Name}}{{end}}{{end}}')
sudo docker run --rm -v ${VOL}:/meili_data alpine rm -rf /meili_data/data.ms
# Verify deletion (prints "data.ms removed" when successful)
sudo docker run --rm -v ${VOL}:/meili_data alpine sh -c '[ ! -e /meili_data/data.ms ] && echo "data.ms removed" || (echo "data.ms still exists" && ls -la /meili_data/data.ms)'
