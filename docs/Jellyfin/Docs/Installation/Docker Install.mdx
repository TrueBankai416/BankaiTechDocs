...
description: Complete guide for installing Jellyfin media server on Ubuntu using Docker with Docker Compose configuration and setup instructions.
image: /img/social/jellyfin-social.webp
tags: [Jellyfin, Media Server, Docker]
keywords: [Jellyfin, Media Server]
last_update:
  author: BankaiTech
updates:
  - date: 2026-08-07
    note: "docs: Update Jellyfin memory limit example from 12G to 8G"
  - date: 2026-08-07
    note: "docs: add OOM troubleshooting and memory limits to Jellyfin/Docker docs"
  - date: 2025-12-20
    note: "Remove sudo from command to create Docker Compose file in installation guide"
  - date: 2025-12-20
    note: "Fix permission command syntax in Docker installation guide"
  - date: 2025-12-20
    note: "Update installation steps for Jellyfin on Docker to set permissions for directories"
---
...
### _Step 2: Create Directory for Jellyfin Files and Set Permissions_

```bash
sudo mkdir /opt/docker && sudo mkdir /opt/docker/jellyfin && sudo chown -R $USER:$USER /opt/docker
```
...
### _Step 3: Create a Docker Compose file for Jellyfin_
...
services:
  jellyfin:
    ...
    # Recommended - prevents Jellyfin from exhausting RAM during trickplay generation or transcoding
    deploy:
      resources:
        limits:
          // highlight-next-line
...
### _Step 5: Complete the Jellyfin Setup Wizard_
...
- **Memory Limits:** Jellyfin can consume large amounts of RAM during trickplay image generation and transcoding. Without a limit, it can exhaust all available RAM and SWAP, causing the kernel OOM killer to terminate other processes. Set `deploy.resources.limits.memory` to a value that leaves headroom for the rest of your system (e.g., `12G` on a 32 GB host).
