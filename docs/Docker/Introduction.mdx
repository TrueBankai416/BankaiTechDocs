...
The `pull_policy` feature requires **Docker Compose V2** (docker compose). The `build` policy option was introduced in Docker Compose specification version 3.8+. For the most up-to-date information, refer to the [official Docker Compose specification](https://docs.docker.com/compose/compose-file/05-services/#pull_policy).
:::
