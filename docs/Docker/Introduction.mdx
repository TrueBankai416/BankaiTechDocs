...
tags: [Docker, Docker Compose, Tutorial]
keywords: [Docker, Docker Engine, Tutorial]
last_update:
  author: BankaiTech
updates:
  - date: 2025-09-21
    note: "Add clarification that pull_policy is ignored for build services"
  - date: 2025-09-21
    note: "CRITICAL FIX: Remove invalid pull_policy: build references"
  - date: 2025-09-21
    note: "Clarify CLI flags table to avoid confusion about --build equivalence"
  - date: 2025-09-21
    note: "Final improvements to Docker Compose pull_policy documentation"
  - date: 2025-09-21
    note: "Add important clarification for build policy usage"
---
...
#### **_Example 1: Simple Custom Network_**
...
#### **_Example 4: Assigning Static IP Addresses_**
...
#### **_Example 5: No Internet Access or Communication_**
...
#### **_Example 10: network_mode: service:[service name]_**
...
#### **_Example 11: container:[container name/id]_**
...
#### **_Docker Compose Example with `tmpfs` Options_**
...
The `pull_policy` feature requires **Docker Compose V2** (docker compose). This section uses the modern `docker compose` CLI syntax. If you're using the legacy `docker-compose` (with hyphen), adjust commands accordingly. For the most up-to-date information, refer to the [official Docker Compose specification](https://docs.docker.com/compose/compose-file/05-services/#pull_policy).
:::
...
Docker Compose supports three distinct pull policy options:
...
services:
  service-name:
    image: nginx:latest
    pull_policy: always  # Options: missing, always, never
```
...
The `always` policy ensures you get the latest version of the image every time by re-pulling from the registry. Note that this re-pulls the same tag (avoiding stale local copies) but doesn't change the tag/version itself:
...
While `pull_policy` controls when to pull pre-built images, building images from source is handled differently:

:::note Important
`pull_policy` is ignored when a service defines `build:`, since Docker Compose builds the image locally instead of pulling it from a registry.
:::

#### **When to Build Instead of Pull**
...
services:
  app:
    build:
      ...
      pull: true  # Pull newer base images during build
    ports:
      - "3000:3000"
...
**Force rebuilds with CLI:**

```bash
# Force rebuild all services with build contexts
docker compose up --build

# Build without using cache
docker compose build --no-cache

# Pull base images during build
docker compose build --pull
```

:::note Important
Building is controlled by defining a `build:` context in your service, not by `pull_policy`. If both `image:` and `build:` are present, Compose builds the image and tags it with the specified image name. Use `build.pull: true` to pull newer base images during the Dockerfile build process.
:::
...
services:
  app:
    build: .  # Fails if Dockerfile missing or context incorrect
```
...
### **Pull Policy vs CLI Flags**

Docker Compose provides command-line flags that can override pull_policy settings or control building:

#### **Pull Policy Overrides**

| CLI Flag         | Equivalent pull_policy | When to Use                                  |
| ---------------- | ---------------------- | -------------------------------------------- |
| `--pull always`  | `pull_policy: always`  | One-time deployments requiring latest images |
| `--pull missing` | `pull_policy: missing` | Default behavior override                    |
| `--pull never`   | `pull_policy: never`   | Offline deployments or testing               |

#### **Build Control (Independent of pull_policy)**

| CLI Flag     | Purpose                                    | When to Use                           |
| ------------ | ------------------------------------------ | ------------------------------------- |
| `--build`    | Force rebuild services with build contexts | Development/testing with code changes |
| `--no-cache` | Build without using cache                  | Clean builds after dependency changes |

#### **File-level vs Command-line Settings**

**Use `pull_policy` in compose files when:**

- You want consistent behavior across all deployments
- Different services need different pull strategies
- Setting up automated deployments or CI/CD pipelines
- Documenting intended deployment behavior

**Use `--pull` CLI flags when:**

- Making one-time deployments with special requirements
- Overriding file settings for testing or debugging
- Testing different pull behaviors

**Use `--build` CLI flags when:**

- Force rebuilding during development
- Ensuring fresh builds after code changes
- One-time builds without modifying compose files

#### **Examples**

```bash
# Override all services to always pull latest
docker compose up --pull always

# Force rebuild all services with build contexts
docker compose up --build

# Combine pull and build flags
...
# Monitor Docker events for image activity (pulls, builds, etc.)
...
- [Docker Compose Specification - pull_policy](https://docs.docker.com/compose/compose-file/05-services/#pull_policy)
