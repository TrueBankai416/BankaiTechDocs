...
- Force rebuilds even when an image already exists (similar to `docker compose up --build`)

:::note Important
The `build` policy only applies to services that define a `build:` context. Services with only an `image:` property will ignore this policy. Additionally, `pull_policy: build` controls when to rebuild the service image, but pulling newer base images during the Dockerfile build process is controlled separately using the `build.pull` option:
...
services:
  app:
    ...
    pull_policy: build  # Always rebuild the service image
```
:::
...
Docker Compose also provides command-line flags that can override or complement pull_policy settings:

| CLI Flag | Related Setting | When to Use |
|----------|----------------|-------------|
...
| `--build` | `pull_policy: build` | Force rebuild for development/testing* |

*Note: `--build` triggers rebuilds for services with build contexts and is conceptually similar to `pull_policy: build`.
