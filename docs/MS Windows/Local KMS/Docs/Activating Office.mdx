...
description: Activate Microsoft Office volume license editions internally using a local KMS server, no individual Microsoft connection required.
image: /img/social/kms-activate-office-social.svg
tags: [Office, KMS]
keywords: [Office, KMS]
last_update:
  author: BankaiTech
...
- `C:\Program Files\Microsoft Office\Office16` for Office 2016 (64-bit)
...
Verify that the Office Installation is registered and shows `Activated`, if it is not `Activated` then repeat steps 4 and 5.
