...
updates:
  - date: 2026-08-28
    note: "docs(gdid): add local account switch steps and reference link"
  - date: 2026-07-26
    note: "docs(gdid): add credit for original script and include references section"
  - date: 2026-07-26
...
**Option A — Switch your current account to a local account (recommended)**

This converts your existing Windows login from a Microsoft account to a local account, so you keep the same profile without needing a second account:

1. Open **Settings** → **Accounts** → **Your info**
2. Click **"Stop signing into Microsoft apps automatically"** (or **"Sign in with a local account instead"** on some builds)
3. Follow the prompts to set a local username and password

After this, your account no longer relies on `wlidsvc` to stay signed in.

**Option B — Create a separate local administrator account**

If you prefer to keep your Microsoft account linked and just want a local fallback:
...
- [Change user account back to local or dissociate it from Microsoft Account](https://learn.microsoft.com/en-us/answers/questions/5520793/change-user-account-back-to-local-or-at-least-asso)
