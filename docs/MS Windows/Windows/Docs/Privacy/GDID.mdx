...
| Option | Description |
|--------|-------------|
| **1. Disable GDID / Telemetry** | Runs the full remediation: stops and disables CDP services, applies Activity Feed group policies, removes residual registry identity entries, blocks network endpoints in the hosts file and Windows Firewall, and installs persistence scheduled tasks. |
...
To create a local account via Settings: **Settings → Accounts → Other users → Add account → "I don't have this person's sign-in information" → "Add a user without a Microsoft account"**
