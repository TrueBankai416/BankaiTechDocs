...
services:
  qbittorrent-vpn:
    ...
    volumes:
      ...
      - /media/merged/downloads:/downloads   # Downloads directory
      - /media/merged:/media                 # Media storage
...
# Create the Docker network (if using external network)
...
2. **Default Save Path**: `/downloads` (mapped to your host directory)
3. **Keep incomplete torrents in**: `/downloads/incomplete`
4. **Copy .torrent files to**: `/downloads/torrents`
