...
:::warning TOC Overlap Issue
If you notice that your floating buttons are being covered by the Table of Contents (TOC) on the right side of the page, you may need to apply a CSS fix to prevent the TOC from extending to the bottom of the viewport.
:::

#### Symptoms
- The last 2-3 TOC entries overlap with your floating buttons
- Discord, Buy Me a Coffee, or other bottom-right buttons are partially or fully hidden
...
**Step 1:** Open your custom CSS file at [src/css/custom.css](../../../src/css/custom.css)
...
This fix works by:
1. **Setting max-height** - Limits the TOC height to `calc(100vh - 200px)`, leaving 200px of space at the bottom for widgets
2. **Adding overflow-y: auto** - Makes the TOC scrollable when content exceeds the max height
3. **Adding padding-bottom** - Provides additional buffer space to ensure all TOC entries are accessible

#### Adjusting the Space

If you need more or less space for your buttons, adjust the `200px` value:
- **More space for buttons**: Increase to `250px` or `300px`
- **Less space for buttons**: Decrease to `150px` or `100px`
- **Formula**: Total space = button height + margins + comfortable buffer
