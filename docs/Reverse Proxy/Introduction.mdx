...
```mermaid
graph TD
    A[Client Browser<br/>Public IP: 203.0.113.45] -->|1. HTTPS Request<br/>https://app.example.com:443<br/>DNS resolves to 198.51.100.10| B[Reverse Proxy Server<br/>Nginx/Caddy/Apache<br/>Public IP: 198.51.100.10<br/>Ports 80, 443]
    B -->|2. Forward Request<br/>http://192.168.1.100:3000| C[Backend Service 1<br/>Web Application<br/>IP: 192.168.1.100<br/>Port 3000]
    C -->|3. HTTP Response<br/>192.168.1.100:3000 → 198.51.100.10:443| B
    B -->|4. HTTPS Response<br/>SSL Terminated<br/>198.51.100.10:443 → 203.0.113.45| A
    
    subgraph "Internet"
        A
        F[DNS Server<br/>app.example.com → 198.51.100.10<br/>api.example.com → 198.51.100.10]
    end
    
    subgraph "Server Infrastructure - Private Network 192.168.1.0/24"
        B
        C
        D[Backend Service 2<br/>API Server<br/>IP: 192.168.1.101<br/>Port 3001]
        E[Backend Service 3<br/>Database Interface<br/>IP: 192.168.1.102<br/>Port 8081]
    end
    
    B -.->|Load Balance<br/>http://192.168.1.101:3001<br/>api.example.com requests| D
    B -.->|Load Balance<br/>http://192.168.1.102:8081<br/>db.example.com requests| E
    
    A -.->|DNS Query<br/>app.example.com| F
    F -.->|DNS Response<br/>198.51.100.10| A
