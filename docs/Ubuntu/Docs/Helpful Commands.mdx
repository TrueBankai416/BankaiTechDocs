...
:::note[mkvpropedit + VC-1 + old mkvmerge]
Files muxed with very old versions of mkvmerge (e.g. v5.8.0 from 2012) and containing VC-1 video can grow indefinitely when `mkvpropedit` tries to rewrite the seek head, because the seek head was written without reserving void space. Use `mkvinfo` to confirm the codec and mkvmerge version before running `mkvpropedit`.
:::
