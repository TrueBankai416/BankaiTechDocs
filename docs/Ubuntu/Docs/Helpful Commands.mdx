...
updates:
  - date: 2026-09-02
    note: "docs(ubuntu): add Windows to Ubuntu file transfer instructions for SCP, WSL, and Git Bash"
  - date: 2026-08-30
    note: "docs(ubuntu): add rename tool section with util-linux and Perl versions"
  - date: 2026-08-26
    note: "docs(ubuntu): add sed -n line range print command"
  - date: 2026-08-23
    note: "docs(ubuntu): add cmp -n flag and dd+xxd+diff forensic workflow"
  - date: 2026-08-23
    note: "docs(ubuntu): add cmp command"
  - date: 2026-08-23
    note: "docs(ubuntu): add diff and mkvmerge -J commands"
---
...
| Package | Command syntax | Supports regex |
|---|---|---|
...
- `-n` — **dry run**: print what would be renamed without making any changes.
- `s/pattern/replacement/` — Perl substitution expression; `g` at the end replaces all occurrences per filename.

:::tip[Prefer the for loop if Perl rename is unavailable]
...
- `-n` — **suppress** the default behavior of printing every line. Without this flag, `sed` prints all lines plus the matched ones.
...
:::tip[Inspecting the differing region — full workflow]
...
|---|---|
| `dd if=/dev/sda of=disk.img bs=4M status=progress` | Clone an entire disk to an image file |
| `dd if=disk.img of=/dev/sdb bs=4M status=progress` | Restore an image file to a disk |
| `dd if=/dev/zero of=testfile bs=1M count=1024 status=progress` | Create a 1 GB zero-filled file (useful for write speed testing) |
| `dd if=/dev/urandom of=testfile bs=1M count=100` | Fill a file with random data |

:::danger[`of=` targets are overwritten silently]
...
:::note[Run SCP from the machine sending the files]
...
**From WSL (type `bash` or `wsl` in PowerShell):**
In WSL, local Windows drives are mounted under `/mnt/` (e.g. `C:\` becomes `/mnt/c/`):
```bash
rsync -avhP "/mnt/c/Users/John/Documents/MEGA downloads/Season 01" server@192.168.1.10:/media/merged/Downloads/
```

**From Git Bash (right-click in the folder → *Open Git Bash here*):**
In Git Bash, local Windows drives are mounted directly under `/` (e.g. `C:\` becomes `/c/`):
```bash
# If launched from outside the folder (using absolute path):
rsync -avhP "/c/Users/John/Documents/MEGA downloads/Season 01" server@192.168.1.10:/media/merged/Downloads/

# If launched using "Open Git Bash here" inside the folder (using relative path):
rsync -avhP "./Season 01" server@192.168.1.10:/media/merged/Downloads/
```
