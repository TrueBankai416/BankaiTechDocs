...
# If installed via install script, updates are manual or via re-running the script
```
...
# Manual update check via CLI (supported on macOS):
tailscale update
```
...
# Manual update check via CLI (supported on Windows):
tailscale update

# If installed via package manager:
# Chocolatey: choco upgrade tailscale
# Scoop: scoop update tailscale
# Winget: winget upgrade tailscale
```
...
# macOS/Windows (built-in updater)
tailscale update
...
# DNF: journalctl -u dnf

# Manual reinstall if update fails (Linux direct installation)
curl -fsSL https://tailscale.com/install.sh | sh

# For macOS/Windows, try the built-in updater or re-download installer
tailscale update  # If supported on your platform
```
...
brew uninstall tailscale         # Remove Homebrew version
