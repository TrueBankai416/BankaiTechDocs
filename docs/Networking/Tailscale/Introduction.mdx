...
# Use exit node with specific routing
tailscale set --exit-node=100.64.0.1 --exit-node-allow-lan-access
...
tailscale set --accept-dns --dns=1.1.1.1,8.8.8.8

# Use split DNS
tailscale set --accept-dns --dns=192.168.1.1 --dns-search=local.domain
...
# No additional firewall config needed for Tailscale
...
**Problem**: Magic DNS overrides local DNS resolution
```bash
# Before Tailscale
myapp.example.com → Public IP (203.0.113.1)

# After Tailscale with Magic DNS
...
# Or use split DNS
tailscale set --accept-dns --dns=1.1.1.1 --dns-search=ts.net
```
...
```bash
# Configure Tailscale to only handle .ts.net domains
tailscale set --accept-dns --dns=1.1.1.1,8.8.8.8 --dns-search=ts.net

# This allows:
# - Public domains → Public DNS
...
#### **Solution 4: Network Namespace Isolation**
```bash
# Run Tailscale in separate network namespace
sudo ip netns add tailscale
sudo ip netns exec tailscale tailscale up

# This isolates Tailscale from main network stack
```
...
```bash
# Configure DNS to avoid conflicts
tailscale set --accept-dns --dns=1.1.1.1 --dns-search=ts.net,local
```
...
# Disable key expiry
tailscale set --auth-key-expiry=false
