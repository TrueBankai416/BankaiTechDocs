...
```bash
# View available exit nodes (check admin console or use status)
tailscale status --json | jq '.Peer[] | select(.ExitNodeOption == true) | {Name: .DNSName, IP: .TailscaleIPs[0]}'

# Use a specific exit node
tailscale set --exit-node=100.64.0.1

# Use exit node with LAN access
tailscale set --exit-node=100.64.0.1 --exit-node-allow-lan-access=true
...
```bash
# Add specific routes for Tailscale traffic
